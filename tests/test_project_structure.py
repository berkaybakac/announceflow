#!/usr/bin/env python3
"""
Project Structure Verification Suite
Tests to ensure project structure is correct and nothing is broken.
"""
import sys
import os
...
def test_gitignore_exists():
    ...
    print("TEST 3: .gitignore exists...", end=" ")
    try:
...
def test_requirements_dev_exists():
    """Test that requirements-dev.txt exists with code quality tools."""
    ...
    try:
        ...
        # Check for black and flake8 (code quality tools)
        if "black" not in content:
            print("❌ FAIL: Missing black")
            return False

        if "flake8" not in content:
            print("❌ FAIL: Missing flake8")
            return False
...
def run_all_tests():
    ...
    results.append(test_gitignore_exists())
